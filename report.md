...
Our tests also simulate a client carrying causal history across connections or sessions. Before issuing an operation from another context, we advance its `operation_time` and `cluster_time` from the previous session. MongoDB explicitly supports this mechanism for establishing causal consistency across sessions.

An important caveat: causal consistency sessions alone do not guarantee the full set of client-centric consistency models. MongoDB's documentation specifies that the complete durable causal-consistency guarantees—read-your-writes, monotonic reads, monotonic writes, and writes-follow-reads—apply only with `readConcern: "majority"` and `writeConcern: "majority"`. Other read concern and write concern combinations provide weaker guarantees, particularly under network partitions and rollback scenarios. This is precisely why our tests deliberately use combinations like `local/w:1` and `majority/w:1`: we are testing the boundaries where the weaker guarantees permit violations.
